...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    data = {
        ...
        "newMessage": {"role": "user", "parts": [{"text": "What menus are available?"}]},
        "streaming": True,
    }
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    message = Message(
        ...
        role=Role.user,
        parts=[Part(root=TextPart(text="Show catering menus"))],
    )
...
def test_reasoning_engine_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    response = requests.post(
        ...
        json={
            ...
            "input": {"user_id": f"u-{uuid.uuid4()}", "message": "Get catering options"},
        },
        stream=True,
        timeout=60,
    )
